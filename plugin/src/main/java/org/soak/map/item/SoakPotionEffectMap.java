...
public class SoakPotionEffectMap {
    ...
    public static org.bukkit.potion.PotionEffect toBukkit(PotionEffect effect) {
        return new org.bukkit.potion.PotionEffect(toBukkit(effect.type()), (int) effect.duration().ticks(), effect.amplifier(), effect.isAmbient(), effect.showsIcon());
    }
    ...
    public static PotionEffectType toSponge(org.bukkit.potion.PotionEffectType bukkitType) {
        var key = ResourceKey.minecraft(bukkitType.getName().toLowerCase());
        return RegistryTypes.POTION_EFFECT_TYPE.get().findValue(key).orElseThrow(() -> new RuntimeException("Cannot find mapping for " + bukkitType.getName()));
    }

    public static org.bukkit.potion.PotionEffectType toBukkit(PotionEffectType type) {
    ...
    public static PotionType toSponge(org.bukkit.potion.PotionType bukkitType) {
        return RegistryTypes.POTION_TYPE.get().findValue(ResourceKey.minecraft(bukkitType.name().toLowerCase())).orElseThrow(() -> new RuntimeException("Could not find sponge potion type of " + bukkitType.name()));
    }

    public static org.bukkit.potion.PotionType toBukkit(PotionType type) {
