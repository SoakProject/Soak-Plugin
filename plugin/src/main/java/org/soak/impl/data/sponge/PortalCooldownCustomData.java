...
public class PortalCooldownCustomData {
    ...
    public static void createTickScheduler() {
        Sponge.asyncScheduler().executor(SoakPlugin.plugin().container()).scheduleAtFixedRate(() -> Sponge
                .server()
                .worldManager()
                .worlds()
                .stream()
    ...
    static Key<Value<Ticks>> generateKey() {
        return Key.builder()
                ...
                .key(ResourceKey.of(SoakPlugin.plugin().container(), "portal_cooldown"))
                .build();
    }

    static void init(RegisterDataEvent event) {
        var resourceKey = ResourceKey.of(SoakPlugin.plugin().container(), "portal_cooldown");
